"""Per-minute API request metrics and the api.forkmesh.com landing page.
...
GROUP_LIMIT = 200
# Path positions that carry caller-chosen identifiers (owner/repo/id …);
# every segment after these prefixes at the listed count is masked.
_MASKED_SEGMENTS = {
    "repo": 2,
    "orgs": 1,
    "org": 1,
...
def _masked_group(path):
    ...
    for index, segment in enumerate(segments[:4]):
        ...
        safe = (
            len(segment) <= 24
            and segment == lowered
            and all(c.isalpha() or c in "-_" for c in segment)
        )
...
async def _flush(env, now):
    ...
    args = []
    for (minute, group, cls), (count, dur_sum, dur_max) in batch.items():
