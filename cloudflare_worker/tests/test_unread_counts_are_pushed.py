...
QT_ACTIONS = (REPO / "qt_client" / "src" / "MainWindowActions.cpp").read_text(
    encoding="utf-8")
QT_REPOS = (REPO / "qt_client" / "src" / "MainWindowRepos.cpp").read_text(
    encoding="utf-8")
QT_SETUP = (REPO / "qt_client" / "src" / "MainWindowSetup.cpp").read_text(
    encoding="utf-8")
QT_CHAT = (REPO / "qt_client" / "src" / "MainWindowChat.cpp").read_text(
    encoding="utf-8")
QT_INTERNAL = (REPO / "qt_client" / "src" / "MainWindowInternal.h").read_text(
    encoding="utf-8")
DOC = (REPO / "docs" / "operations" / "polling-elimination.md").read_text(
    encoding="utf-8")
...
def test_a_failed_first_read_is_retried_rather_than_stranding_the_badge():
    ...
    assert (
        "            // The one read of this run just failed, so there is "
        "nothing to sit\n"
        "            // on. Clearing the stamp lets the next heartbeat "
        "re-seed it (and\n"
