...
QT_MAIN = ROOT / "qt_client" / "src" / "MainWindow.cpp"
...
def _load_account_login(extra_globals):
    ...
    selected = [
        ...
        if isinstance(node, ast.AsyncFunctionDef) and node.name == "_account_login"
    ]
...
def _login_harness(rec):
    ...
    return handler, saved, login_clears, login_fails
...
def test_login_binds_desktop_pubkey_to_unbound_password_account():
    handler, saved, login_clears, login_fails = _login_harness(_active_account())
...
def test_login_refuses_to_silently_replace_existing_desktop_pubkey():
    handler, saved, _login_clears, login_fails = _login_harness(
    ...
    assert response == {"status": 409, "data": {"error": "pubkey_mismatch"}}
    assert saved == []
    assert login_fails == []


def test_login_without_desktop_pubkey_still_allows_plain_web_session():
    handler, saved, _login_clears, login_fails = _login_harness(_active_account())
