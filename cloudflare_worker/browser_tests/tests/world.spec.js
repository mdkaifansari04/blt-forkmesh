...
test("the full avatar front and belt mirror the live cabinet lights", async ({
  page,
}) => {
  test.setTimeout(45_000);
  await prepareWorldPage(page, "avatar-node-belt", {
    session: {
      nodeName: "jett",
      sessionToken: "avatar-node-belt-session",
    },
    directoryUsers: [
      {
        name: "jett",
        nodes: ["mirror2", "mirror3"],
      },
    ],
  });
  await waitForWorld(page);
  ...
  const state = await page.locator("forkmesh-world").evaluate((shell) => {
    ...
    shell.world.updateNetworkNodes(nodes);
    shell.world.updateIdentity({ nodes });
    const player = shell.world.player;
    const badge = player.userData.badge;
    const belt = player.getObjectByName("forkmesh-operator-belt");
    const beltLights = [];
    belt.traverse((object) => {
      if (!object.userData?.mirrorStatusLight) return;
      beltLights.push({
        name: object.userData.nodeName,
        color: `#${object.material.color.getHexString()}`,
        blink: object.userData.blink,
      });
    });
    const cabinetLights = [];
    shell.world.scene.traverse((object) => {
      ...
      const lens = object.userData.signalRing?.children?.[1];
      cabinetLights.push({
        name: object.userData.nodeRecord?.name,
        color: lens ? `#${lens.material.color.getHexString()}` : "",
        blink: Boolean(object.userData.beaconSweep),
      });
    });
    return {
      badge: {
        width: badge.geometry.parameters.width,
        height: badge.geometry.parameters.height,
        y: badge.position.y,
        bottom: badge.position.y - badge.geometry.parameters.height / 2,
      },
      beltY: belt.children[0].position.y,
      beltLights,
      cabinetLights,
    };
  });

  expect(state.badge.width).toBeCloseTo(1.64, 4);
  expect(state.badge.height).toBeCloseTo(1.64, 4);
  expect(state.badge.y).toBeCloseTo(2.35, 4);
  expect(state.badge.bottom).toBeCloseTo(1.53, 4);
  expect(state.badge.bottom).toBeGreaterThan(state.beltY);
  expect(state.beltLights).toEqual([
    { name: "belt-online", color: "#00cc44", blink: false },
    { name: "belt-degraded", color: "#ff0000", blink: true },
  ]);
  expect(
    state.cabinetLights.sort((left, right) =>
  ...
  if (process.env.FORKMESH_WORLD_PREVIEW_PATH) {
    const shell = page.locator("forkmesh-world");
    await shell.evaluate((element) => {
      element.world.setCameraZoom(0.2);
    });
    await page.waitForTimeout(500);
    await shell.evaluate((element) => {
      element.world.setPaused(true);
      const player = element.world.player;
      const camera = element.world.camera;
      player.lookAt(
        camera.position.x,
        player.position.y,
        camera.position.z,
      ...
      player.userData.targetHeading = player.rotation.y;
    });
    await page.screenshot({
      path: process.env.FORKMESH_WORLD_PREVIEW_PATH,
      animations: "disabled",
    });
  }
});
