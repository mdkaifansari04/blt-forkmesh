...
// Bootstrap writes the minimum native mirror-node state for a fresh server.
// Repository bytes are deliberately not part of installation: the daemon's
// first reconciliation clones from the round-robin upstream and retries on its
// normal sync interval when the network is temporarily empty.
