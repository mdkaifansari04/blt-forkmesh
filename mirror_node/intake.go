...
// daemon.
//
// The bridge is purely push-driven: the Daemon forwards it the events from
// the shared per-owner node event channel (see Daemon.handleNodeEvent) and it
// starts the worker when an "issues"/"pulls"/"discussions" event lands for
// the intake repository. It never calls GET /api/repo/*/pending — that poll
// (five seconds fleet-wide) was the busiest endpoint on the relay while
// reading a ten-minute edge cache, and it is gone with no fallback poll
// behind it. Missed-push safety comes from the socket itself: every reconnect
// emits one catch-up wake, so work queued while the channel was down is
// drained as soon as it is back.
type IntakeBridge struct {
	program   string
	repoKey   string // "owner/repository", lowercased, for event filtering
	idleGrace time.Duration
	// onWorkerExit runs after each worker exit (the Daemon republishes the
	// possibly-drained refs promptly instead of waiting for the heartbeat).
	onWorkerExit func()
...
// handleEvent receives every frame from the shared node event channel. A
// catch-up wake (fired once per successful connect) and an inbox-topic frame
// for the intake repository both mark work pending; frames for other
// repositories, and topics the worker does not drain (e.g. "commits", which
// the Daemon's own sync handles), are ignored.
