...
func (d *Daemon) hasExternalUpstreams() bool {
	...
	relayHosts := []string{catalog.Host}
	switch strings.ToLower(catalog.Host) {
	case "app.forkmesh.com", "api.forkmesh.com", "forkmesh.com":
		relayHosts = []string{
			"app.forkmesh.com", "api.forkmesh.com", "forkmesh.com",
		}
	default:
		if strings.HasPrefix(strings.ToLower(catalog.Host), "api.") {
			relayHosts = append(relayHosts, catalog.Host[len("api."):])
		}
	}
	for _, upstreams := range d.config.Upstreams {
		for _, upstream := range upstreams {
			...
			for _, relayHost := range relayHosts {
				if strings.EqualFold(parsed.Host, relayHost) {
					internal = true
					break
				}
			}
