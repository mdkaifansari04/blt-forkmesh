...
def _stamped_site_outputs(root, versions):
    return {
        rel: dashboard_shell.stamp_asset_versions(
            (root / rel).read_text(encoding="utf-8"), versions)
        for rel in site_page_assets(root)
    }
...
def main():
    ...
    if legacy.exists():
        legacy.unlink()
        changed.append("app/public/dashboard.html (removed)")
    if changed:
