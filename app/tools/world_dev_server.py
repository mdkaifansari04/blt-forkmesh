...
Serves the checkout's ``world/public/world/`` files (no caching, so edits show on
...
Run it from anywhere inside the repo:
    ...
    python3 app/tools/world_dev_server.py
...
https://app.forkmesh.com), and ``--world-dir``. Stdlib only; no dependencies.
"""
...
DEFAULT_PORT = 8788
DEFAULT_UPSTREAM = "https://app.forkmesh.com"
DEFAULT_WORLD_DIR = (
    Path(__file__).resolve().parents[2] / "world" / "public" / "world"
)
