    ...
    container.innerHTML = `
      ${entries.length
        ? `<div class="grid gap-1">${entries.map((entry) => {
            const repo = entry.repo;
            // The building icon already marks an organization entry, so the
            // rail spells it out only for screen readers: the literal word
            // "ORGANIZATION" was wide enough to truncate the repository name
            // it was labelling down to "owasp-bl…" in this 270px column.
            return `<a href="${escapeHtml(entry.href)}" class="flex min-w-0 items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm text-muted-foreground hover:bg-secondary hover:text-foreground">
              ...
              <span class="min-w-0 flex-1 truncate">${escapeHtml(repoDisplayKey(repo))}</span>
              ${repositoryTermsBadge(repo, true)}
              ${entry.organization ? '<span class="sr-only">organization repository</span>' : ""}
            </a>`;
          }).join("")}</div>`
