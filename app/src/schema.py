...
SCHEMA_STATEMENTS = [
    ...
    # At-most-once ownership of each minute's status sample. Replayed alarm
    # delivery may call record_status_sample again for the same minute. The
    # daily/hourly rollups are checks-counter increments, so whichever attempt
    # INSERTs this minute's row first owns the sample; later attempts skip it
    # instead of double-counting the hour. `claim` is a random token the winner
    # reads back to recognize itself (D1's Python client exposes no reliable
    # changes() count). Pruned alongside system_status_minute.
    ...
    # Email invitations to an organization (migration 0124). An org
    # owner/admin invites an address; the recipient follows a single-use
    # signed link and is joined to org_members at the stored role. Unlike the
    # plaintext org roster tables above, an invitee's address is PII belonging
    # to someone who may never accept, so this follows the
    # contributor_invitations shape: a blind index for lookup, dedupe and rate
    # limiting, everything else inside the encrypted blob. Only sha256(token)
    # is retained in the blob; the raw token exists in the delivered email and
    # nowhere else.
    ...
    # One outstanding invite per (org, address): the concurrency boundary.
    ...
    # Per-minute traffic buckets behind the app /api diagnostics
