...
def test_mirror_nodes_table_has_separate_owner_column():
    ...
    assert '{MirrorNodeColNode, "device-desktop", "Node"}' in build
    assert '{MirrorNodeColOwner, "person", "Owner"}' in build
    assert '{MirrorNodeColCommit, "git-commit", "Latest commit"}' in build
...
def test_mirror_nodes_show_the_latest_commit_message_and_author():
    ...
    assert '{MirrorNodeColCommit, "git-commit", "Latest commit"}' in build
    assert '{MirrorNodeColMessage, "note", "Message"}' in build
    assert '{MirrorNodeColAuthor, "pencil", "Author"}' in build
...
def test_mirror_nodes_header_labels_cover_every_column():
    ...
    labels = _body(
        ...
        "static_assert(sizeof(kMirrorHeaderIcons)",
    )
    mappings = [
        line for line in labels.splitlines()
        if line.strip().startswith("{MirrorNodeCol")
    ]
    assert len(mappings) == len(columns), (
        f"{len(columns)} columns but {len(mappings)} header mappings"
    )
    assert '{MirrorNodeColNode, "device-desktop", "Node"}' in labels
    assert '{MirrorNodeColSync, "sync", "Sync"}' in labels
    assert '{MirrorNodeColOwner, "person", "Owner"}' in labels
    assert mappings[-1].strip() == (
