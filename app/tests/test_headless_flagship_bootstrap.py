...
WINDOW = ROOT / "desktop" / "src" / "MainWindow.cpp"
SHARED = ROOT / "desktop" / "src" / "MainWindowShared.cpp"
MAIN = ROOT / "desktop" / "src" / "main.cpp"
...
def test_mirror_advert_cache_includes_namespace_and_publish_state():
    ...
    start = repos.index("void MainWindow::refreshMirrorAdverts()")
    end = repos.index("void MainWindow::mirrorAdvertisedRepo", start)
    body = repos[start:end]

    assert "QString inputSignature;" in body
    assert "repo.owner + QLatin1Char('|') + repo.name + QLatin1Char('|')" in body
    assert "repo.cloneUrl + QLatin1Char('|')" in body
    assert "QString::number(repo.publishToNetwork)" in body
    assert "inputSignature == m_mirrorAdvertInputSig" in body
    assert "m_mirrorAdvertSig = inputSignature" in body
...
def test_mirror_publish_requires_a_served_commit_and_sockets_are_retired():
    ...
    assert "startNodeEventSocket();" in start_hosts
