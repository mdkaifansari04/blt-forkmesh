...
LATEST_UPDATE_SLUG = "one-app-one-mesh"
...
def test_blog_page_indexes_every_feature_post():
    ...
    assert len(posts) == 77
    assert html.count('class="blog1-card" href="/blog/') == 77
    assert html.count('class="blog1-search-result" href="/blog/') == 77
    assert html.count('data-blog-search-text=') == 77
    assert 'href="/blog/one-app-one-mesh/"' in html
...
def test_latest_product_update_has_production_metadata_art_and_navigation():
    ...
    previous = _read(
        PUBLIC / "blog" / "safer-rooms-steadier-mirrors" / "index.html"
    )

    for marker in (
        '<link rel="canonical" href="https://forkmesh.com/blog/one-app-one-mesh/">',
        '<meta property="article:published_time" content="2026-08-08T00:00:00-04:00">',
        '<meta property="og:image:width" content="1672">',
        '<meta property="og:image:height" content="941">',
        'src="/assets/blog/features/one-app-one-mesh.webp"',
        '<span class="pill">August 8, 2026</span>',
    ...
    assert index.index('href="/blog/one-app-one-mesh/"') < index.index(
        'href="/blog/safer-rooms-steadier-mirrors/"'
    )
    assert (
        'href="/blog/one-app-one-mesh/">← One app, one mesh'
        in previous
    )
