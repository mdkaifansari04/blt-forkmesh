...
def test_every_first_party_script_carries_the_shared_content_hash():
    versions = _versions()
    seen = set()
    for root, rel in _stamped_documents():
        html = (root / rel).read_text(encoding="utf-8")
        for name, query, version in SCRIPT_SRC_RE.findall(html):
            if name in UNVERSIONED_SCRIPTS:
                assert not query, (root, rel, name)
                continue
            # A bundle referenced by a page but missing from the single source
            # of truth would silently ship uncacheable-busted.
            assert name in dashboard_shell.CACHE_BUSTED_BUNDLES, (root, rel, name)
            assert version == versions[name], (root, rel, name)
            seen.add(name)
