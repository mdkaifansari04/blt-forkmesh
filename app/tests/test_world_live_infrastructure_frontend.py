...
APP = (ROOT.parent / "world" / "public" / "world" / "world.js").read_text(encoding="utf-8")
...
def test_status_board_counts_down_each_second_and_fetches_once_per_minute():
    ...
    assert "world-admin-status-spin" in (ROOT.parent / "world" / "public" / "world" / "world.css").read_text(encoding="utf-8")
    ...
    assert "if (!loading && remaining <= 0 && this.worldBoardsCircleNear)" in timer
    assert (
