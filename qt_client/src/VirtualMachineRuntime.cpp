...
void appendMount(QStringList &mounts, const QString &path)
{
    ...
    // If a broader root is discovered later, retain only that root. This keeps
    // the generated Lima configuration minimal and the exposed surface obvious.
                ...
                const QString &limactlOverride)
{
    ...
    // Include a configuration generation in the name so a future boundary
    // change cannot silently reuse an older instance with broader mounts.
    fresh.name =
...
void applyGuestEnvironmentPolicy(QProcessEnvironment &environment)
{
    ...
    // Lima needs a few host values to find its instance and SSH client. Strip
    // every other value before launching it, then use Lima's own block/allow
    // filter for the guest boundary. This remains fail-closed if an older Lima
    // build were ever invoked accidentally.
