...
void MainWindow::startVoiceCapture()
{
    ...
    if (!m_voiceLiveTimer) {
        ...
        // Re-transcribe roughly every second so dictated words land in the box soon
        // after they're spoken. Ticks where the clip hasn't grown are skipped in
        // startVoiceTranscription(), so this stays cheap while you pause.
...
void MainWindow::startVoiceTranscription(bool finalPass)
{
    ...
    // fallback (-nf, which otherwise re-decodes "hard" segments several times), and
    // most of the box's cores (-t) while leaving one for the UI so the app stays
    // smooth during transcription.
    ...
    proc->start(binary, {QStringLiteral("-m"), model, QStringLiteral("-f"), wav,
                         ...
                         QStringLiteral("1"), QStringLiteral("-nf"),
