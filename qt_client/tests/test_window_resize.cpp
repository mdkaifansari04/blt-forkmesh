...
int main(int argc, char *argv[])
{
    ...
    // used to be gated on the Code/Branches tab being on screen — which it isn't,
    // the default landing tab is Agents — so the status strip's bottom-left
    // branch button and the branch count stayed on their empty-repo values for
    // good, and the lazily-loaded tab badges kept the zero they were built with.
    // Open a repo with no refs at all, then let the clone land: the next
    // push-driven refresh must catch all of it up.
    {
