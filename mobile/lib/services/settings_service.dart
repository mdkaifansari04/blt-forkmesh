...
class SettingsService extends ChangeNotifier {
  ...
  static const productionServerUrl =
      'wss://app.forkmesh.com/api/repo/mainnode/forkmesh/rooms/world-general/ws';
  static const localWorkerServerUrl =
      'ws://localhost:8787/api/repo/mainnode/forkmesh/rooms/world-general/ws';
  static const androidEmulatorLocalWorkerServerUrl =
      'ws://10.0.2.2:8787/api/repo/mainnode/forkmesh/rooms/world-general/ws';
  ...
  static const defaultRoom = 'world-general';
  ...
  String get serverUrl =>
      canonicalServerUrl(_prefs.getString('server/url') ?? defaultServerUrl);
  ...
  static String canonicalServerUrl(String value) {
    final parsed = Uri.tryParse(value.trim());
    if (parsed == null || !const {'ws', 'wss'}.contains(parsed.scheme)) {
      return value;
    }
    const legacyPath = '/api/repo/mainnode/forkmesh/rooms/general/ws';
    const currentPath = '/api/repo/mainnode/forkmesh/rooms/world-general/ws';
    final canonicalHost =
        const {
          'forkmesh.com',
          'api.forkmesh.com',
        ...
        parsed.scheme == 'wss' &&
        !parsed.hasPort;
    final sharedRoom = parsed.path == legacyPath || parsed.path == currentPath;
    if (!sharedRoom) return value;
    return parsed
        .replace(
          host: canonicalHost ? 'app.forkmesh.com' : parsed.host,
          path: currentPath,
        )
        .toString();
  }
  ...
  static Uri worldUriForServerUrl(String value) {
    ...
    if (relay == null ||
      ...
      return Uri.https('world.forkmesh.com', '/');
    }
    if (const {
      'forkmesh.com',
      'api.forkmesh.com',
      'app.forkmesh.com',
    }.contains(relay.host.toLowerCase())) {
      return Uri.https('world.forkmesh.com', '/');
    }
    return Uri(
