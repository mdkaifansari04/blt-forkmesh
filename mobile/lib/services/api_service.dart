...
class ApiService {
  ...
  Future<List<PublishedPull>> publishedPulls(String owner, String name) {
    ...
    return _cached(_pullsCache, key, () async {
      final dirs = await _numberedFolders(owner, name, '.forkmesh/pulls');
      final items = await Future.wait(
        dirs.map((dir) async {
          try {
            final b = await blob(owner, name, '.forkmesh/pulls/$dir/pull.md');
  ...
  Future<List<PullEvent>> _pullEvents(
    ...
    try {
      final t = await tree(owner, name, path: '.forkmesh/pulls/$number');
      final eventFiles =
          t.entries
              .where(
                (e) =>
