  ...
  test(
    ...
    () async {
      ...
      final subscription = server.listen((request) async {
        request.response.headers.contentType = ContentType.json;
        final path = request.uri.queryParameters['path'] ?? '';
        if (request.uri.path.endsWith('/tree') && path == '.forkmesh/pulls') {
          request.response.write(
            jsonEncode({
              'entries': [
                {'name': '7', 'path': '.forkmesh/pulls/7', 'type': 'dir'},
              ],
            }),
          );
        } else if (request.uri.path.endsWith('/tree') &&
            path == '.forkmesh/pulls/7') {
          request.response.write(
            jsonEncode({
              'entries': [
                {
                  'name': 'pull.md',
                  'path': '.forkmesh/pulls/7/pull.md',
                  'type': 'file',
                },
                {
                  'name': '001-comment.md',
                  'path': '.forkmesh/pulls/7/001-comment.md',
                  'type': 'file',
                },
                {
                  'name': '002-review.md',
                  'path': '.forkmesh/pulls/7/002-review.md',
                  'type': 'file',
                },
              ],
            }),
