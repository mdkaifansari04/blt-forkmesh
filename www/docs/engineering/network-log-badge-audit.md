...
| List | Where |
| --- | --- |
| classification rules (substring → badge) | `desktop/src/MainWindowSettings.cpp:4060` (`kNetworkLogRules`) |
| quick-filter chip order | `desktop/src/MainWindowSettings.cpp:4531` (`order[]`) |
| legend icons + accents | `desktop/src/MainWindowInternal.h:7661` (`categories()`) |
...
1. **Static** — extract the message text of all 690 distinct string-literal log
   messages passed to `logSystem()` / `flashMessage()` across `desktop/src`,
...
bounded relay sync."* (`desktop/src/RepoHost.cpp:73`), and nothing connects to
...
`2026-08-06 12:45:01  Mirror sync finished  [desktop/src/MainWindowRepos.cpp:812]`
...
(`forkmesh::logMessageBody()`, `desktop/src/LogSource.h`): the path is not
