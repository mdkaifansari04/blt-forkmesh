...
| Poller | Was | Now |
| --- | --- | --- |
| Go mirror node intake bridge (`server/intake.go`) | `GET /api/repo/*/pending` every 5s per node — the busiest endpoint on the relay | Push-only: one `EventSocket` (`server/nodeevents.go`) to `wss://…/api/nodes/events`; worker starts on push, stops `intakeIdleGrace` after the last one. `intakePollInterval` is parsed but ignored. No fallback poll. |
...
All in `world/public/world/world.js` (constants near the top);
