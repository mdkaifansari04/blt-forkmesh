...
`www/docs/mirror-gateway-config.schema.json`. A Tunnel-backed endpoint manifest uses
`www/docs/mirror-gateway-manifest.schema.json`; it records a proxied CNAME and
...
`desktop/src/PrivateMirrorStore.cpp` is the private-repository storage
