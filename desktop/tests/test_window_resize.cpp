...
void checkCloudLogMonitorAlert(MainWindow &window)
{
    ...
    window.testCloudLogMonitorLine(QStringLiteral(
        ...
        "\"method\":\"GET\",\"url\":\"https://app.forkmesh.com/api/status\","
        "\"headers\":{\"user-agent\":\"Mozilla/5.0 (X11; Linux) Chrome/126\"}},"
        "\"response\":{\"status\":200}}}"));
    QApplication::processEvents();
    const QStringList afterHealthyHit = window.testNetworkLog();
    const QStringList healthyCloudHits = afterHealthyHit.filter(
        QStringLiteral("Cloud hit: GET https://app.forkmesh.com/api/status"));
    check(!window.testErrorBorderVisible() &&
              window.testCloudLogMonitorEvents() == 1 &&
              healthyCloudHits.size() == 1 &&
              healthyCloudHits.constFirst().contains(
                  QStringLiteral("Cloud hit: GET https://app.forkmesh.com/api/"
                                 "status")) &&
              healthyCloudHits.constFirst().contains(
    ...
    window.testCloudLogMonitorLine(QStringLiteral(
        ...
        "\"request\":{\"method\":\"POST\",\"url\":\"https://app.forkmesh.com/api/"
    ...
    window.testCloudLogMonitorChunk(QByteArray(
        ...
        "        \"request\": {\n"
        "            \"method\": \"GET\",\n"
        "            \"url\": \"https://app.forkmesh.com/api/repos\",\n"
        "            \"headers\": {\n"
    ...
    const QStringList splitWorkerErrors = afterSplitEvent.filter(
    ...
    check(window.testCloudLogMonitorEvents() == eventsBefore + 1 &&
              ...
              window.testErrorBorderVisible() &&
              splitWorkerErrors.size() == 1 &&
              splitWorkerErrors.constFirst().contains(
...
void checkCloudLogMerged(MainWindow &window)
{
    ...
    window.testCloudLogMonitorLine(QStringLiteral(
        ...
        "\"request\":{\"method\":\"GET\",\"url\":\"https://app.forkmesh.com/api/"
...
int main(int argc, char *argv[])
{
    ...
    check(window.findChild<QPushButton *>(
              QStringLiteral("siteDeployButton")) != nullptr &&
              window.findChild<QPushButton *>(
                  QStringLiteral("siteDeployAppButton")) != nullptr &&
              window.findChild<QPushButton *>(
    ...
    QLabel *fleetHealthSummary = window.findChild<QLabel *>(
        QStringLiteral("hostsFleetHealthSummary"));
    check(fleetHealthSummary &&
              window.findChild<QPushButton *>(
                  QStringLiteral("hostsProbeAllButton")) != nullptr &&
              fleetHealthSummary->text().contains(QStringLiteral("saved")),
          QStringLiteral(
              "Hosts shows an aggregate fleet health summary and an immediate recheck action"));
    ...
    {
        check(forkmesh::ui::buildScratchDir(QStringLiteral("/opt/forkmesh/desktop/build")) ==
                  QStringLiteral("/opt/forkmesh/desktop/build/.forkmesh-tmp"),
    ...
    // brew prefixes) so only assert the deterministic checkout-and-build steps here.
    {
        ...
        check(freshSteps.value(3) ==
                  QStringLiteral("cmake --build /tmp/preview/acme-app-pr7/desktop/build "
                                 "-j 4"),
    ...
    const QString retiredAccountInstance =
        QStringLiteral("wss://api.forkmesh.com/api/repo/mainnode/forkmesh/"
                       "rooms/world-general/ws");
    const QString remoteAccountInstance =
        QStringLiteral("wss://relay.example.test/api/repo/mainnode/forkmesh/"
                       "rooms/world-general/ws");
    QSettings().setValue(
        QStringLiteral("account/recentUserInstances"),
        QJsonDocument(QJsonObject{
                          {QStringLiteral("alice"), retiredAccountInstance},
                          {QStringLiteral("bob"), remoteAccountInstance}})
            .toJson(QJsonDocument::Compact));
    check(window.testRecentUserInstance(QStringLiteral("alice")) ==
                  forkmesh::ui::kDefaultServerUrl,
          QStringLiteral("account switching migrates a saved API instance to app"));
    const QJsonObject rewrittenAccountInstances =
        QJsonDocument::fromJson(
            QSettings()
                .value(QStringLiteral("account/recentUserInstances"))
                .toByteArray())
            .object();
    check(rewrittenAccountInstances.value(QStringLiteral("alice")).toString() ==
                  forkmesh::ui::kDefaultServerUrl &&
              window.testRecentUserInstance(QStringLiteral("bob")) ==
                  remoteAccountInstance &&
              rewrittenAccountInstances.value(QStringLiteral("bob")).toString() ==
                  remoteAccountInstance,
          QStringLiteral("account switching rewrites only retired official instances"));

    const QString sessionSecret = QStringLiteral("v2.window-only.session-secret");
    const QStringList settingsKeysBeforeSessionCache = QSettings().allKeys();
    window.testCacheAccountSession(
        QStringLiteral(" Alice "), forkmesh::ui::kDefaultServerUrl,
        sessionSecret, true, QStringLiteral("desktop-key"));
    window.testCacheAccountSession(
        QStringLiteral("bob"), remoteAccountInstance,
        QStringLiteral("v2.remote.window-secret"), false, QString());
    check(window.testAccountSessionCacheSize() == 2 &&
              window.testCachedAccountSession(
                  QStringLiteral("alice"), forkmesh::ui::kDefaultServerUrl) ==
                  sessionSecret &&
              window.testCachedAccountSession(
                  QStringLiteral("alice"), remoteAccountInstance).isEmpty(),
          QStringLiteral("the avatar switcher keeps independent in-memory "
                         "sessions per account and exact instance"));
    check(window.testCachedAccountSessionMaySendTo(
                  QStringLiteral("alice"), forkmesh::ui::kDefaultServerUrl,
                  QUrl(QStringLiteral(
                      "https://app.forkmesh.com/api/accounts/profile"))) &&
              !window.testCachedAccountSessionMaySendTo(
                  QStringLiteral("alice"), forkmesh::ui::kDefaultServerUrl,
                  QUrl(QStringLiteral(
          ...
          QStringLiteral("the avatar switcher never sends a cached bearer to "
                         "another origin or a downgraded transport"));
    check(window.testCachedAccountSessionMayRestoreDesktop(
                  QStringLiteral("alice"), forkmesh::ui::kDefaultServerUrl,
                  QStringLiteral("desktop-key"),
                  QStringLiteral("desktop-key")) &&
              !window.testCachedAccountSessionMayRestoreDesktop(
                  QStringLiteral("alice"), forkmesh::ui::kDefaultServerUrl,
                  QStringLiteral("desktop-key"),
              ...
              !window.testCachedAccountSessionMayRestoreDesktop(
                  ...
                  QStringLiteral("desktop-key"),
                  QStringLiteral("desktop-key")),
          QStringLiteral("quick switching restores signing only after the "
                         "same local key is revalidated"));
    bool sessionReachedSettings =
        QSettings().allKeys() != settingsKeysBeforeSessionCache;
    for (const QString &key : QSettings().allKeys()) {
        const QVariant value = QSettings().value(key);
        sessionReachedSettings =
            sessionReachedSettings || value.toString().contains(sessionSecret) ||
            value.toByteArray().contains(sessionSecret.toUtf8());
    }
    check(!sessionReachedSettings,
          QStringLiteral("quick-switch bearer tokens never enter QSettings"));
    window.testClearAccountSessionCache();
    check(window.testAccountSessionCacheSize() == 0,
          QStringLiteral("application teardown clears quick-switch sessions"));
    ...
    // and that origin is a link into the Files explorer.
    {
        ...
        static const QRegularExpression originRe(
            QStringLiteral("  \\[desktop/src/[A-Za-z0-9._/-]+:[0-9]+\\]$"));
        ...
        check(window.testLogBadgeFor(QStringLiteral(
                  ...
                  "[desktop/src/MainWindowIssues.cpp:12]")) ==
                  QStringLiteral("SAVE"),
        ...
        if (QTextBrowser *logView = window.testNetworkLogView()) {
            ...
            for (QTextBlock b = logView->document()->firstBlock(); b.isValid();
                 b = b.next()) {
                if (!b.text().contains(QStringLiteral("Origin click check")))
                    continue;
    ...
    // and the worktree identity beside both history and the footer branch.
    {
        ...
        if (controls) {
            ...
            for (QPushButton *button : buttons) {
                if (button->property("buttonSize") == "xs")
                    ++compactActionCount;
            }
            compactActions = compactActionCount == 3;
        }
        check(compactActions,
              QStringLiteral("the three commit actions use the shorter compact size"));

        QPushButton *autoViewEye = nullptr;
        if (scmPanel) {
        ...
        check(autoViewEye && autoViewEye->isCheckable(),
              QStringLiteral("working-tree auto-view has an explicit eye toggle"));
    ...
    check(window.testAgentListChromeHidden(),
          QStringLiteral("agents list ships with no column header and no frame "
                         "border (adhoc #92)"));
    // The complete fleet toolbar floats over the session list's top-right
    ...
    // and queue controls together.
    {
        ...
        QPushButton *hideDetail = nullptr;
        if (queueOverlay) {
            ...
            for (QPushButton *button : iconButtons) {
                if (button->parentWidget() == queueOverlay) {
                    hideDetail = button;
                    break;
                }
            }
        }
        const bool queueOverlayContract =
            queueStatus && decrease && increase && startAll && stopAll &&
        ...
        check(queueOverlayContract,
              QString("Agents fleet controls float together at the list top "
                      "with terminal launchers and queue controls "
                      "(overlay=%1 parent=%2 geometry=%3,%4 %5x%6 queue=%7)")
                  .arg(queueOverlay != nullptr)
                  .arg(queueOverlay && queueOverlay->parentWidget() != nullptr)
                  .arg(queueOverlay ? queueOverlay->x() : -1)
                  .arg(queueOverlay ? queueOverlay->y() : -1)
                  .arg(queueOverlay ? queueOverlay->width() : -1)
                  .arg(queueOverlay ? queueOverlay->height() : -1)
                  .arg(queueStatus ? queueStatus->text()
    ...
    window.testClickRepoDetailTab(5);
    QApplication::processEvents();
    ...
    if (initGitRepo(wtRepo)) {
        ...
        QWidget *gitFooterDock = window.findChild<QWidget *>(
            QStringLiteral("logDock"));
        QWidget *gitPromptHost = window.findChild<QWidget *>(
            QStringLiteral("promptOverlayHost"));
        check(window.testPromptOverlayPlacement() == QStringLiteral("anchored") &&
                  gitFooterDock && gitPromptHost &&
                  gitPromptHost->parentWidget() == gitFooterDock &&
                  gitPromptHost->geometry().center().x() >
                      gitFooterDock->rect().center().x() &&
                  gitPromptHost->geometry().bottom() ==
                      gitFooterDock->rect().bottom(),
        ...
        // rather than pasted (or blindly chopped two files in).
        {
            QStringList many{"-C", "/home/f/projects/forkmesh", "diff", "main",
                             "--"};
            for (int i = 0; i < 291; ++i)
                many << QStringLiteral(":(literal)app/tests/"
                                       "test_world_%1.py")
                            .arg(i);
            const QString crumb =
        ...
        QApplication::processEvents();
        const QString agentPullCounts = gitOutput(
            wtRepo.path(),
            {"rev-list", "--left-right", "--count",
             "main..." + agentRouteBranch});
        const QString agentRouteHead = gitOutput(
            wtRepo.path(), {"rev-parse", agentRouteBranch}).trimmed();
        check(agentPullCounts.startsWith(QLatin1Char('1')) &&
                  agentRouteHead == agentRouteBaseRef &&
                  window.testBranchPullEnabled(),
              QString("opening an agent branch is read-only and exposes its "
                      "explicit Pull main action (main...branch = %1)")
                  .arg(agentPullCounts));
    ...
    // just as the assign-an-agent path jumps straight to the new session.
    {
        ...
        if (initGitRepo(quickAddRepo)) {
            ...
            // Quick-adding one (no agent) must select the freshly-created issue
            // and leave its detail pane visible.
            const int number = qaWindow.testQuickAddIssueNoAgent(
                QStringLiteral("Land me on the detail pane"));
            QApplication::processEvents();

            check(number > 0 && qaWindow.testIssueDetailVisible(),
                  QString("quick-add without an agent opens the new issue's "
                          "detail pane (number=%1 visible=%2) "
                          "(#203)")
                      .arg(number)
    ...
    // issue #154: references inside an issue/PR comment body become in-app links.
    {
        ...
        check(ClaudeTranscriptView::linkifyReferences(
                  QStringLiteral("open desktop/src/ClaudeTranscriptView.cpp:1445")) ==
                  QStringLiteral("open [desktop/src/ClaudeTranscriptView.cpp:1445]"
                                 "(forkmesh-file:desktop%2Fsrc%2FClaudeTranscriptView.cpp?line=1445)"),
              QStringLiteral("agent transcript links a repo file at a line"));
        check(ClaudeTranscriptView::linkifyReferences(
                  QStringLiteral("open /repo/desktop/src/MainWindow.cpp#L42")) ==
                  QStringLiteral("open [/repo/desktop/src/MainWindow.cpp#L42]"
                                 "(forkmesh-file:%2Frepo%2Fdesktop%2Fsrc%2FMainWindow.cpp?line=42)"),
    ...
    // session's transcript search so hits highlight in place.
    {
        ...
        window.testClickRepoDetailTab(3);
        QApplication::processEvents();
    ...
    // that really failed still is, including a 200 carrying {"ok":false,…}.
    {
        ...
        window.testLogSystem(QString::fromUtf8(
            ...
            "proof\"}]}] https://app.forkmesh.com/api/notifications?node=jett "
            "\xC2\xB7 ping inbox"));
        QStringList stored = window.testNetworkLog();
        check(!stored.isEmpty() &&
        ...
        window.testLogSystem(QString::fromUtf8(
            ...
            "https://app.forkmesh.com/api/notifications \xC2\xB7 ping inbox"));
        stored = window.testNetworkLog();
        check(!stored.isEmpty() &&
                  window.testLogBadgeFor(stored.last()) == QStringLiteral("ERROR"),
        ...
        window.testLogSystem(QString::fromUtf8(
            ...
            "https://app.forkmesh.com/api/repo/jett/forkmesh \xC2\xB7 repo fetch"));
        stored = window.testNetworkLog();
        check(!stored.isEmpty() &&
                  window.testLogBadgeFor(stored.last()) == QStringLiteral("ERROR"),
        ...
        window.testResetNetworkLog();
        window.testLogSystem(QString::fromUtf8(
            "net GET ERR 503 https://app.forkmesh.com/api/repo/forkmesh/forkmesh"
        ...
        window.testLogSystem(QString::fromUtf8(
            ...
            "https://app.forkmesh.com/api/repo/jett/forkmesh/mirrors "
            "\xC2\xB7 mirror sync"));
        stored = window.testNetworkLog();
        check(!stored.isEmpty() &&
                  window.testLogBadgeFor(stored.last()) != QStringLiteral("ERROR"),
