...
// AgentRunner authenticates each relay turn with the per-run Ed25519 ticket in
// FORKMESH_AI_AGENT_AUTH. The embedded agent uses only the Python standard
// library and executes tools inside the session worktree.
...
# Cloudflare rejects the default Python-urllib signature before Worker routing.
USER_AGENT = "ForkMesh-AI-agent/1.0 (+https://forkmesh.com)"
RATE_LIMIT_RETRIES = 3
RATE_LIMIT_MAX_WAIT = 120
# Retry transient Worker and network failures with bounded backoff.
TRANSIENT_RETRIES = 4
TRANSIENT_MAX_WAIT = 30
# Stop after repeated turns containing only previously executed commands.
...
def prepare_git_environment():
    ...
    Point GIT_CONFIG_GLOBAL at a scratch file seeded with the identity git
    already resolves here, keeping global writes inside the run.
    """
    identity = []
...
def command_key(command):
    ...
    Compare shell tokens so equivalent quoting cannot bypass the repeat guard.
    A command shlex cannot parse falls back to its stripped text.
    """
    try:
...
def main():
    ...
    for turn in range(1, MAX_TURNS + 1):
        ...
        # Accept a no-tool response only when the worktree is clean.
